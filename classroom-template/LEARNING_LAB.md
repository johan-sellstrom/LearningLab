...
Setup shortcuts (recommended)
- macOS: `./scripts/bootstrap-mac.sh`
- Windows: `powershell -ExecutionPolicy Bypass -File .\\scripts\\bootstrap-windows.ps1`
- No local installs: use GitHub Codespaces / VS Code Dev Containers (this repo includes `.devcontainer/`)
- Docker (issuer+verifier only; wallet lessons are out-of-container): `docker compose up --build`
...
- iProov SDK
  ...
  - Android: https://github.com/iProov/android
- OHTTP
...
- `wallet-ios/`: Notes for iProov integration point, BBS+ toggle, OHTTP client wrapper (iOS)
- `wallet-android/`: Notes for iProov integration point, BBS+ toggle, OHTTP client wrapper (Android)
